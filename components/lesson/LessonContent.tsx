import {
  getCourseLanguageMeta,
  getOptionPhrase,
  getPhrasePrimaryText,
  getPhraseSecondaryText,
  getPhraseWords,
  getQuestionCourseLanguage,
  getQuestionPrompt,
  Question,
  SpeakingOption,
...
const getFunctionErrorMessage = async (error: unknown): Promise<string> => {
  ...
  if (functionError.context) {
    try {
      const errorBody = await functionError.context.clone().json();
      console.error("Transcription function error body:", errorBody);
      ...
      try {
        const errorText = await functionError.context.clone().text();
        console.error("Transcription function error text:", errorText);

        if (errorText) {
          return errorText;
        }
      } catch {
        // Fall back to the Supabase SDK message below.
      }
    }
  }

  return functionError.message || "Could not transcribe audio.";
};

const normalizePhrase = (
  phrase?: ReturnType<typeof getQuestionPrompt> | ReturnType<typeof getOptionPhrase>,
): NormalizedPhrase => {
  const breakdownValue =
    phrase && typeof phrase.breakdown === "string" ? phrase.breakdown : "";

  return {
    hanzi: getPhrasePrimaryText(phrase) || "",
    pinyin: getPhraseSecondaryText(phrase) || "",
    words: getPhraseWords(phrase),
    breakdown: breakdownValue,
  };
};

const normalizeOption = (option: SpeakingOption): NormalizedOption =>
  ({
    ...option,
    mandarin: normalizePhrase(getOptionPhrase(option)),
  }) as NormalizedOption;

const normalizeQuestion = (question: Question): NormalizedQuestion =>
  {
    const language = getQuestionCourseLanguage(question);
    const languageMeta = getCourseLanguageMeta(language);

    return {
    ...question,
    language: {
      id: language,
      title: languageMeta.title,
      primaryLabel: languageMeta.primaryLabel,
      secondaryLabel: languageMeta.secondaryLabel,
      speechLocale: languageMeta.speechLocale,
    },
    mandarin: normalizePhrase(getQuestionPrompt(question)),
    options: question.options.map((option) =>
      normalizeOption(option as SpeakingOption),
    ),
...
export default function LessonContent({
  ...
  const currentQuestion = useMemo<NormalizedQuestion>(
    () => normalizeQuestion(rawCurrentQuestion),
    [rawCurrentQuestion],
  );
  ...
  const selectedSentence = useMemo((): NormalizedOption | null => {
    ...
    return (option as NormalizedOption | undefined) ?? null;
  ...
  const playAudio = () => {
    ...
    Speech.speak(textToSpeak, {
      language: currentQuestion.language.speechLocale,
      onDone: () => {
        setIsSpeechPlaying(false);
        finishListening();
      },
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      const audioFormat = Platform.OS === "android" ? "m4a" : "wav";
      const base64Audio = await FileSystem.readAsStringAsync(uri, {
        encoding: FileSystem.EncodingType.Base64,
      });
      console.log("Submitting audio for transcription:", {
        format: audioFormat,
        language: currentQuestion.language.id,
  ...
  const handleRevealPromptText = () => {
    if (showPromptText) {
      Animated.timing(fadeAnim, {
        toValue: 0,
        duration: 250,
        useNativeDriver: true,
      }).start(() => setShowPromptText(false));
    } else {
            ...
            if (q.type === "listening_mc") {
              ...
              english =
                normalizedQuestion.options.find(
                  (opt) => opt.id === normalizedQuestion.correctOptionId,
                )?.english || "";
              hanzi = normalizedQuestion.mandarin.hanzi;
              pinyin = normalizedQuestion.mandarin.pinyin;
            } else {
              const option = normalizeQuestion(q).options[0];
      ...
      <View style={styles.content}>
        ...
        >
          <AudioPrompt
            ...
            onRevealPhrase={handleRevealPromptText}
            currentQuestion={currentQuestion}
            showPhrase={showPromptText}
          ...
          >
            {currentQuestion.type === "multiple_choice" && (
              <MultipleChoiceMode
                ...
                languageTitle={currentQuestion.language.title}
                selectedOption={selectedOption}
      ...
      {currentQuestion.type === "listening_mc" &&
        ...
        hasListenedToAudio && (
          <SentenceBreakdownCard
            ...
            primaryLabel={currentQuestion.language.primaryLabel}
            secondaryLabel={currentQuestion.language.secondaryLabel}
            speechLocale={currentQuestion.language.speechLocale}
            disabled={showResult}
          />
        )}
      {currentQuestion.type !== "listening_mc" &&
