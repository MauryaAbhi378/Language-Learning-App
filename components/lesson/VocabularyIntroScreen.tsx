...
interface StudyCard {
  key: string;
  word: Word;
  direction: "target-to-en" | "en-to-target";
}
  ...
  questions.forEach((question) => {
    const wordSource =
      question.type === "listening_mc"
        ? getPhraseWords(getQuestionPrompt(question))
        : question.options.flatMap((opt) => getPhraseWords(getOptionPhrase(opt)));

    wordSource.forEach((word) => {
      const wordKey = getWordKey(word);

      if (wordKey && !allWords.has(wordKey)) {
        allWords.set(wordKey, word);
      }
    });
  });
...
export default function VocabularyIntroScreen({
  ...
  useEffect(() => {
    setState(initializeStudyState(deck));
  }, [deck]);
